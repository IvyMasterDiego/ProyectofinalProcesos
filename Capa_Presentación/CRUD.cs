...
namespace Capa_Presentación
{
    public partial class CRUD : Form
    {
                ...
                try
                {
                    if (bEdit == false)
                    {
                        if (objectCN.RegisterExists(txtDNI.Text, query) == false)
                            ...
                            MessageBox.Show("El DNI que intenta ingresar ya existe", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    }
                    else
                    {
                    ...
                    MessageBox.Show("Proceso completado", "Succeed", MessageBoxButtons.OK, MessageBoxIcon.Information);
                    CleanFields();
                    ShowPayroll();
                }
                catch (Exception ex)
            ...
            switch (bEdit)
            {
                case false:
                    if (dgvPayroll.SelectedRows.Count > 0)
                    {
                        ...
                        MessageBox.Show("Debe de seleccionar una fila para poder realizar los cambios", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    break;

                case true:
                    if (MessageBox.Show("Desea cancelar la edicion?", "Action confirmation", MessageBoxButtons.YesNo, MessageBoxIcon.Warning) == DialogResult.Yes)
                    {
                        txtDNI.Enabled = true;
                        btnEdit.Text = "Edit";
                        bEdit = false;
        ...
        private void btnDelete_Click(object sender, EventArgs e)
        {
            if (dgvPayroll.SelectedRows.Count > 0)
            {
                if (MessageBox.Show("Desea eliminar esta fila", "Action confirmation", MessageBoxButtons.YesNo, MessageBoxIcon.Warning) == DialogResult.Yes)
                {
                    DNI = dgvPayroll.CurrentRow.Cells[0].Value.ToString();
                    objectCN.DeletePayroll(DNI);
                    MessageBox.Show("Registo removido correctamente");
                    ShowPayroll();
                }
            }
            else
                MessageBox.Show("Debe seleccionar una fila para poder eliminarla", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
        }

        // Logout button
        private void btnLogout_Click(object sender, EventArgs e)
        {
            if (MessageBox.Show("Desea cerrar sesion?", "Action confirmation", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
            {
                this.Close();
                LoginW.Show();
            }
